...
{
  /**
    @brief Detects which TMT labelling kit (plex) was used in an LC-MS/MS experiment.

    Given an MSExperiment with MS2 spectra, the algorithm quantifies the reporter-ion region
    of each MS2 spectrum against the union of all reporter ions of all supported TMT/TMTpro kits
    (TMT 6/10/11/16/18/32/35-plex) and decides, per kit, how well the observed channel pattern
    matches that kit. It returns a probability for each kit (sorted, highest first) and identifies
    the most parsimonious kit (the one with the fewest channels that still explains all channels
    ...
    TMT 32-plex), which is computed at run time from the shared, high-accuracy channel masses in
    TMTMasses.h (see buildHierarchy()). The hierarchy makes the parsimony decision robust: the
    chosen kit is the smallest kit whose channel set covers every present channel.
  ...
  */
  class OPENMS_DLLAPI TMTPlexDetection
  {
  public:
    ...
    struct OPENMS_DLLAPI Parameters
    {
      ...
      /// a channel counts as 'present' if its population fraction is >= present_fraction * (max population fraction over all channels)
    ...
    /// Result for one candidate TMT kit.
    struct OPENMS_DLLAPI KitResult
    {
      MethodType type = MethodType::UNKNOWN; ///< the kit's MethodType
      std::string name;                      ///< human-readable kit name, e.g. "TMT 16-plex (TMTpro)"
    ...
    /**
      @brief Detect the TMT kit used in @p exp.
    ...
    */
    ...
    /// Build the subset/superset hierarchy of all supported TMT kits from their (shared) channel masses.
    /// Currently, this will spell out as: TMT6 ⊂ TMT10 ⊂ TMT11 ⊂ TMT16 ⊂ {TMT18, TMT32} ⊂ TMT35
    static std::vector<HierarchyNode> buildHierarchy();

    /// The TMT kits considered by detect() and buildHierarchy(), ordered from fewest to most channels.
