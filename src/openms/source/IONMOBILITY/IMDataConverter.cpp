...
std::vector<PeakMap> IMDataConverter::splitByFAIMSCV(PeakMap&& exp)
{
  ...
  if (CVs.empty()) { throw Exception::MissingInformation(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, "Not FAIMS data!"); }
                                                                                                        ...
                                                                                                        MZ_UNITS mz_binning_width_unit)
{
  ...
  if (range_IM.getSpan() / number_of_bins < bin_extension_abs * 2)
  {
    throw Exception::InvalidValue(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION,
                                  String("Bin size (") + String(range_IM.getSpan() / number_of_bins) + ") is smaller than the overlap.",
                                  String(bin_extension_abs * 2));
  }
...
bool IMDataConverter::getIMUnit(const DataArrays::FloatDataArray& fda, DriftTimeUnit& unit)
{
  ...
  { // fallback for non-standard IM arrays (as created by Mobi-DIK)
    if (fda.getName().hasSubstring("MS:1002815")) { unit = DriftTimeUnit::VSSC; }
    else { unit = DriftTimeUnit::MILLISECOND; }
    return true;
  }
  try
  {
    ...
    if (cv.isChildOf(cv_term.id, "MS:1002893")) // is child of generic 'ion mobility array'?
    {
      ...
      { // fallback
        OPENMS_LOG_WARN << "Warning: FloatDataArray for IonMobility data '" << cv_term.id << " " << cv_term.name << "' does not contain proper units!"
                        << std::endl;
