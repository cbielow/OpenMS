...
MSSpectrum& MSSpectrum::select(const std::vector<Size>& indices)
{
  ...
  for (Size i = 0; i < float_data_arrays_.size(); ++i)
  {
    ...
    if (float_data_arrays_[i].size() != peaks_old)
    {
      throw Exception::Precondition(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION,
                                    "FloatDataArray[" + String(i) + "] size (" + String(float_data_arrays_[i].size())
                                      + ") does not match spectrum size (" + String(peaks_old) + ")");
    }
  ...
  for (Size i = 0; i < string_data_arrays_.size(); ++i)
  {
    ...
    {
      throw Exception::Precondition(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION,
                                    "StringDataArray[" + String(i) + "] size (" + String(string_data_arrays_[i].size())
                                      + ") does not match spectrum size (" + String(peaks_old) + ")");
    }
  ...
  for (Size i = 0; i < integer_data_arrays_.size(); ++i)
  {
    ...
    {
      throw Exception::Precondition(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION,
                                    "IntegerDataArray[" + String(i) + "] size (" + String(integer_data_arrays_[i].size())
                                      + ") does not match spectrum size (" + String(peaks_old) + ")");
    }
...
SpectrumSettings::SpectrumType MSSpectrum::getType(const bool query_data) const
{
  ...
  {
    if (dp->getProcessingActions().count(DataProcessing::PEAK_PICKING) == 1) { return SpectrumSettings::CENTROID; }
  }

  if (query_data) { return PeakTypeEstimator::estimateType(begin(), end()); }
...
MSSpectrum::ConstIterator MSSpectrum::getBasePeak() const
{
  ...
  {
    if (largest->getIntensity() < current->getIntensity()) { largest = current; }
  }
  return largest;
}
...
{
  return std::accumulate(cbegin(), cend(), 0.0, [](MSSpectrum::PeakType::IntensityType sum, const PeakType& p) { return sum + p.getIntensity(); });
}
...
MSSpectrum::ConstIterator MSSpectrum::MZEnd(MSSpectrum::ConstIterator begin, MSSpectrum::CoordinateType mz, MSSpectrum::ConstIterator end) const
{
  PeakType p;
  p.setPosition(mz);
...
MSSpectrum::ConstIterator MSSpectrum::MZBegin(MSSpectrum::ConstIterator begin, MSSpectrum::CoordinateType mz, MSSpectrum::ConstIterator end) const
{
  PeakType p;
  p.setPosition(mz);
...
Int MSSpectrum::findNearest(MSSpectrum::CoordinateType mz,
                            MSSpectrum::CoordinateType tolerance_left,
...
Int MSSpectrum::findNearest(MSSpectrum::CoordinateType mz, MSSpectrum::CoordinateType tolerance) const
{
  ...
  if (found_mz >= mz - tolerance && found_mz <= mz + tolerance) { return static_cast<Int>(i); }
  else { return -1; }
}
...
Size MSSpectrum::findNearest(MSSpectrum::CoordinateType mz) const
{
  ...
  --it2;
  if (std::fabs(it->getMZ() - mz) < std::fabs(it2->getMZ() - mz)) { return Size(it - ContainerType::begin()); }
  else { return Size(it2 - ContainerType::begin()); }
}

Int MSSpectrum::findHighestInWindow(MSSpectrum::CoordinateType mz,
                                    MSSpectrum::CoordinateType tolerance_left,
                                    MSSpectrum::CoordinateType tolerance_right) const
{
  if (ContainerType::empty()) { return -1; }
...
void MSSpectrum::sortByPositionPresorted(const std::vector<Chunk>& chunks)
{
  ...
  else
  {
    ...
    {
      if (! chunks[i].is_sorted) { std::stable_sort(select_indices.begin() + chunks[i].start, select_indices.begin() + chunks[i].end, comparePos); }
    }
    ...
    rec = [&chunks, &select_indices, &rec, &comparePos](Size first, Size last) -> void {
      if (last > first)
      {
        ...
        rec(first, mid);
        rec(mid + 1, last);
        std::inplace_merge(select_indices.begin() + chunks[first].start, select_indices.begin() + chunks[mid].end,
                           select_indices.begin() + chunks[last].end, comparePos);
      }
    };
...
void MSSpectrum::sortByPosition()
{
  ...
  // sort index list
  sort([this](const Size i1, const Size i2) -> bool { return this->operator[](i1).getPosition() < this->operator[](i2).getPosition(); });
}

void MSSpectrum::sortByIntensity(bool reverse)
{
  if (reverse && std::is_sorted(ContainerType::begin(), ContainerType::end(), [](auto& left, auto& right) {
  ...
  if (float_data_arrays_.empty() && string_data_arrays_.empty() && integer_data_arrays_.empty())
  {
    if (reverse)
    {
      std::stable_sort(ContainerType::begin(), ContainerType::end(), [](auto& left, auto& right) {
        PeakType::IntensityLess cmp;
        return cmp(right, left);
      });
    }
    else { std::stable_sort(ContainerType::begin(), ContainerType::end(), PeakType::IntensityLess()); }
    return;
  }
  ...
  if (reverse)
  {
    this->sort([this](const Size i1, const Size i2) -> bool { return this->operator[](i2).getIntensity() < this->operator[](i1).getIntensity(); });
  }
  else
  {
    this->sort([this](const Size i1, const Size i2) -> bool { return this->operator[](i1).getIntensity() < this->operator[](i2).getIntensity(); });
  };
}
...
#pragma clang diagnostic ignored "-Wfloat-equal"
  return std::operator==(*this, rhs) && RangeManagerType::operator==(rhs) && SpectrumSettings::operator==(rhs)
         && retention_time_ == rhs.retention_time_ && drift_time_ == rhs.drift_time_ && drift_time_unit_ == rhs.drift_time_unit_
         && ms_level_ == rhs.ms_level_ && float_data_arrays_ == rhs.float_data_arrays_ && string_data_arrays_ == rhs.string_data_arrays_
...
MSSpectrum::MSSpectrum(const std::initializer_list<Peak1D>& init): ContainerType(init)
{
}
...
MSSpectrum::Iterator MSSpectrum::MZBegin(MSSpectrum::Iterator begin, MSSpectrum::CoordinateType mz, MSSpectrum::Iterator end)
{
  PeakType p;
  p.setPosition(mz);
  return lower_bound(begin, end, p, PeakType::PositionLess());
...
MSSpectrum::Iterator MSSpectrum::MZEnd(MSSpectrum::Iterator begin, MSSpectrum::CoordinateType mz, MSSpectrum::Iterator end)
{
  PeakType p;
  p.setPosition(mz);
  return upper_bound(begin, end, p, PeakType::PositionLess());
...
MSSpectrum::Iterator MSSpectrum::PosBegin(MSSpectrum::Iterator begin, MSSpectrum::CoordinateType mz, MSSpectrum::Iterator end)
{
...
MSSpectrum::Iterator MSSpectrum::PosEnd(MSSpectrum::Iterator begin, MSSpectrum::CoordinateType mz, MSSpectrum::Iterator end)
{
...
MSSpectrum::ConstIterator MSSpectrum::PosBegin(MSSpectrum::ConstIterator begin, MSSpectrum::CoordinateType mz, MSSpectrum::ConstIterator end) const
{
  return MZBegin(begin, mz, end);
}
...
MSSpectrum::ConstIterator MSSpectrum::PosEnd(MSSpectrum::ConstIterator begin, MSSpectrum::CoordinateType mz, MSSpectrum::ConstIterator end) const
{
  return MZEnd(begin, mz, end);
}
...
bool getIonMobilityArray__(const MSSpectrum::FloatDataArrays& fdas, Size& index, DriftTimeUnit& unit)
{
  ...
  {
    if (IMDataConverter::getIMUnit(fdas[index], unit)) { return true; }
  }
  return false;
}
