...
void MetricMap::pushDataDouble(String Head,vector<double> Data)
{
	if(Columnlength_ == 0 || Columnlength_ == Data.size() )
	{
		DataStringDouble_.insert(DataStringDouble_.end(), pair<String,vector<double>>(Head,Data));
		isfilled_=isfilled_ || true;
		Columnlength_ = Data.size();
	}
	else
...
vector<pair<String,String>> MetricMap::getHeads()//Returns all Heads. With Type of its Data(Sting/Size). I.e:([Proteins ,String],[Length,Size],[NumberOfProteins,Size],..)
{
  ...
  for (map<String,vector<double>>::const_iterator it = DataStringDouble_.begin() ; it != DataStringDouble_.end() ; it ++)
  {
  		output.push_back(pair<String,String>(it->first,"Double"));
  }
  return output;
}
...
vector<double> MetricMap::getDoublesByHead(String WantedHead)
{
  return DataStringDouble_[WantedHead];
}
