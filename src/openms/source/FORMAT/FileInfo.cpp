...
// $Authors: Marc Sturm, Clemens Groepl, Lars Nilse, Chris Bielow, Timo Sachsenberg $
// --------------------------------------------------------------------------
...
namespace OpenMS
{
  namespace // file-local helpers, moved verbatim from the FileInfo TOPP tool
  {
    /// iterate over all items in CONTAINER and use LAMBDA to extract the charge from each item
    ...
    template <class T>
    ostream& operator<<(ostream& os, const Math::SummaryStatistics<T>& rhs)
    {
    ...
    // -- helpers to project ranges into the structured FileInfo::Result ----------

    /// extract a RangeSet from any range-bearing object (map or a RangeManager view)
    template <class RangeT>
    FileInfo::RangeSet extractRangeSet_(const RangeT& m)
    {
      FileInfo::RangeSet rs;
      if (!m.RangeRT::isEmpty())        { rs.rt        = {true, m.getMinRT(),        m.getMaxRT()}; }
      if (!m.RangeMZ::isEmpty())        { rs.mz        = {true, m.getMinMZ(),        m.getMaxMZ()}; }
      if constexpr (std::is_base_of<RangeMobility, RangeT>())
      {
        rs.has_mobility = true;
        if (!m.RangeMobility::isEmpty()) { rs.mobility = {true, m.getMinMobility(), m.getMaxMobility()}; }
      }
      if (!m.RangeIntensity::isEmpty()) { rs.intensity = {true, m.getMinIntensity(), m.getMaxIntensity()}; }
      return rs;
    }

    /// ranges for a non-MSExperiment map (feature / consensus): only `combined` is filled
    template <class Map>
    FileInfo::Ranges extractRanges_(const Map& map)
    {
      FileInfo::Ranges r;
      r.is_experiment = false;
      r.combined = extractRangeSet_(map);
      return r;
    }

    /// ranges for an MSExperiment: combined / spectra / per-MS-level / chromatograms
    inline FileInfo::Ranges extractRangesExp_(const MSExperiment& exp)
    {
      FileInfo::Ranges r;
      r.is_experiment = true;
      r.combined = extractRangeSet_(exp.combinedRanges());
      r.spectra_overall = extractRangeSet_(exp.spectrumRanges());
      for (UInt lvl : exp.spectrumRanges().getMSLevels())
      {
        r.per_ms_level[lvl] = extractRangeSet_(exp.spectrumRanges().byMSLevel(lvl));
      }
      r.chromatograms = extractRangeSet_(exp.chromatogramRanges());
      return r;
    }
  } // anonymous namespace

  std::vector<std::tuple<Int, std::string, UInt64>> FileInfo::PeakInfo::activationMethodsFlat() const
  {
    std::vector<std::tuple<Int, std::string, UInt64>> out;
    out.reserve(activation_methods.size());
    for (const auto& am : activation_methods)
    {
      out.emplace_back(am.first.first, am.first.second, am.second);
    }
    return out;
  }

  FileInfo::Result FileInfo::run(const std::string& filename, const Options& options)
  {
    Result r;
    r.meta.file_name = filename;

    FileTypes::Type in_type = options.forced_type;
    if (in_type == FileTypes::UNKNOWN)
    {
      in_type = FileHandler::getType(filename);
    }
    r.meta.file_type = in_type;
    r.meta.file_type_name = FileTypes::typeToName(in_type);

    if (in_type == FileTypes::UNKNOWN)
    {
      return r; // caller decides how to report an unknown type
    }

    std::ostringstream os;
    std::ostringstream os_tsv;
    report_(filename, in_type, options, os, os_tsv, r);
    r.text = os.str();
    r.tsv = os_tsv.str();
    return r;
  }

  FileInfo::Result FileInfo::runAll(const std::string& filename)
  {
    Options o;
    o.meta = true;
    o.processing = true;
    o.statistics = true;
    return run(filename, o);
  }

  std::string FileInfo::toText(const Result& r, const Options& /*options*/)
  {
    return r.text;
  }

  std::string FileInfo::toTSV(const Result& r, const Options& /*options*/)
  {
    return r.tsv;
  }

  void FileInfo::report_(const std::string& in, FileTypes::Type in_type, const Options& options,
                         std::ostream& os, std::ostream& os_tsv, Result& r)
  {
    ...
    if (options.validate)
    {
      ...
      default:
        os << '\n'
           ...
           << '\n';
        r.validation.performed = true;
        r.validation.supported = false;
        return;
      }
      ...
      if ((in_type == FileTypes::MZML) || (in_type == FileTypes::MZDATA))
      {
        ...
        else
        {
          ...
          for (Size i = 0; i < warnings.size(); ++i)
          {
            ...
            r.validation.warnings.push_back(warnings[i]);
          }
          for (Size i = 0; i < errors.size(); ++i)
          {
            os << "Error: " << errors[i] << '\n';
            r.validation.errors.push_back(errors[i]);
          }
          if (valid)
          {
    ...
    if (in_type == FileTypes::FASTA)
    {
      ...
      // --- structured result (additive; does not affect os / os_tsv) ---
      FastaInfo fa;
      fa.num_sequences = entries.size();
      fa.total_residues = number_of_residues;
      fa.is_nucleic_acid = is_nucleic_acid;
      fa.seq_with_ambiguous = seq_has_ambiguous;
      fa.dup_headers = dup_header;
      fa.dup_sequences = dup_seq;
      // skip zero-count keys (the verbatim print code may operator[]-insert 'N'/'n' for nucleic FASTA)
      for (const auto& rc : residue_counts) { if (rc.second != 0) { fa.residue_counts[rc.first] = rc.second; } }
      if (sequence_lengths.size() >= 3) // SummaryStatistics computes quartiles, which require >= 3 values
      {
        std::vector<double> sl(sequence_lengths.begin(), sequence_lengths.end());
        fa.length_stats = Math::SummaryStatistics<std::vector<double>>(sl);
      }
      else if (!sequence_lengths.empty()) // 1-2 sequences: fill manually (quartiles fall back to min/max)
      {
        std::vector<double> sl(sequence_lengths.begin(), sequence_lengths.end());
        std::sort(sl.begin(), sl.end());
        auto& st = fa.length_stats;
        st.count = sl.size();
        st.min = sl.front();
        st.max = sl.back();
        st.mean = Math::mean(sl.begin(), sl.end());
        st.median = Math::median(sl.begin(), sl.end(), true);
        st.lowerq = st.min;
        st.upperq = st.max;
        st.variance = (sl.size() >= 2) ? Math::variance(sl.begin(), sl.end(), st.mean) : 0.0;
      }
      if (is_nucleic_acid)
      {
        fa.ambiguity_counts["N"]     = count_residues(residue_counts, std::string_view("Nn"));
        fa.ambiguity_counts["IUPAC"] = count_residues(residue_counts, NA_AMBIGUOUS);
      }
      else
      {
        fa.ambiguity_counts["BZX"]  = count_residues(residue_counts, AA_AMBIGUOUS_BXZ);
        fa.ambiguity_counts["BZXJ"] = count_residues(residue_counts, AA_AMBIGUOUS_BXZJ);
      }
      r.fasta = fa;
    }
    else if (in_type == FileTypes::FEATUREXML) //features
    {
      ...
      FeatureInfo finfo;
      finfo.is_consensus = false;
      finfo.num_features = feat.size();
      finfo.tic = tic;
    ...
    else if (in_type == FileTypes::CONSENSUSXML) //consensus features
    {
      ...
      for (const auto& d : cons.getColumnHeaders())
      {
        ...
        col.filename = d.second.filename;
        col.identifier = std::to_string(d.first);
        col.label = d.second.label;
        col.size = d.second.size;
        finfo.map_columns.push_back(col);
      }
      r.feature = finfo;
      r.ranges = extractRanges_(cons);
    }
    else if (in_type == FileTypes::IDXML || in_type == FileTypes::MZIDENTML) //identifications
    {
      ...
      // --- structured result (additive) ---
      IdentInfo idi;
      if (!id_data.proteins.empty())
      {
        idi.db_name = id_data.proteins[0].getSearchParameters().db;
        idi.db_version = id_data.proteins[0].getSearchParameters().db_version;
        idi.taxonomy = id_data.proteins[0].getSearchParameters().taxonomy;
      }
      for (const auto& se : search_engines) { idi.search_engines.push_back(se.first + " (version: " + se.second + ")"); }
      idi.num_runs = runs_count;
      idi.protein_hits = protein_hit_count;
      idi.non_redundant_protein_hits = proteins.size();
      idi.matched_spectra = spectrum_count;
      idi.peptide_hits = peptide_hit_count;
      // real ratio: cast before dividing (the CLI text deliberately prints the integer-divided value)
      idi.psms_per_spectrum = static_cast<double>(average_peptide_hits) / std::max<UInt>(1u, spectrum_count);
      idi.avg_peptide_length = Math::mean(peptide_length.begin(), peptide_length.end());
      idi.non_redundant_peptides = peptides.size();
      idi.modified_tophits = modified_peptide_count;
      for (const auto& m : mod_counts) { idi.modification_counts[m.first] = m.second; }
      r.ident = idi;
    }
    ...
    else if (in_type == FileTypes::MZTAB)
    {
      ...
      // --- structured result (additive) ---
      MzTabInfo mt;
      mt.version = mztab.getMetaData().mz_tab_version.get();
      mt.mode = mztab.getMetaData().mz_tab_mode.get();
      mt.type = mztab.getMetaData().mz_tab_type.get();
      mt.psms = mztab.getNumberOfPSMs();
      mt.peptides = mztab.getPeptideSectionRows().size();
      mt.proteins = mztab.getProteinSectionRows().size();
      mt.oligonucleotides = mztab.getOligonucleotideSectionRows().size();
      mt.osms = mztab.getOSMSectionRows().size();
      mt.small_molecules = mztab.getSmallMoleculeSectionRows().size();
      mt.nucleic_acids = mztab.getNucleicAcidSectionRows().size();
      r.mztab = mt;
    }
    else if (in_type == FileTypes::TRANSFORMATIONXML)
    {
      ...
      // --- structured result (additive) ---
      std::ostringstream tsum;
      tsum << "Transformation model: " << trafo.getModelType() << '\n';
      trafo.printSummary(tsum);
      r.transformation_summary = tsum.str();
    }
    else if (in_type == FileTypes::PQP)
    {
      ...
      // --- structured result (additive) ---
      std::ostringstream pqp_sum;
      pqp_sum << summary;
      r.targeted_summary = pqp_sum.str();
    }
    else // peaks
    {
      ...
      // --- structured result (additive; does not affect os / os_tsv) ---
      r.ranges = extractRangesExp_(exp);
      {
        PeakInfo pk;
        pk.instrument_name = exp.getInstrument().getName();
        for (const auto& ma : exp.getInstrument().getMassAnalyzers())
        {
          pk.mass_analyzers.emplace_back(MassAnalyzer::NamesOfAnalyzerType[static_cast<size_t>(ma.getType())], ma.getResolution());
        }
        for (UInt l : levels) { pk.ms_levels.push_back(static_cast<Int>(l)); }
        pk.total_peaks = exp.getSize();
        pk.num_spectra = exp.size();
        for (const auto& c : counts) { pk.spectra_per_ms_level[static_cast<Int>(c.first)] = c.second; }
        for (UInt l : levels)
        {
          pk.peak_type_per_ms_level[static_cast<Int>(l)] =
            std::string(SpectrumSettings::NamesOfSpectrumType[level_annotated_picked[l]]) + " (" +
            SpectrumSettings::NamesOfSpectrumType[level_estimated_picked[l]] + ")";
        }
        for (const auto& am : act_method_counts)
        {
          pk.activation_methods[{static_cast<Int>(am.first.mslevel), Precursor::NamesOfActivationMethod[static_cast<size_t>(am.first.am)]}] = am.second;
        }
        // precursor charges (first precursor per spectrum, mirroring the CLI charge distribution)
        for (const auto& spec : exp)
        {
          for (const auto& pc : spec.getPrecursors()) { ++pk.precursor_charges[pc.getCharge()]; break; }
        }
        // meta data arrays (name -> #spectra carrying it)
        for (const MSSpectrum& sp : exp)
        {
          for (const auto& a : sp.getFloatDataArrays())   { ++pk.float_arrays[a.getName()]; }
          for (const auto& a : sp.getIntegerDataArrays()) { ++pk.int_arrays[a.getName()]; }
          for (const auto& a : sp.getStringDataArrays())  { ++pk.string_arrays[a.getName()]; }
        }
        // FAIMS compensation voltages
        {
          auto cvs2 = FAIMSHelper::getCompensationVoltages(exp);
          pk.faims_cvs.assign(cvs2.begin(), cvs2.end());
        }
        // chromatograms
        pk.num_chromatograms = exp.getChromatograms().size();
        {
          Size ncp = 0;
          for (const MSChromatogram& ms : exp.getChromatograms())
          {
            ncp += ms.size();
            ++pk.chromatogram_types[ChromatogramSettings::ChromatogramNames[static_cast<size_t>(ms.getChromatogramType())]];
          }
          pk.num_chrom_peaks = ncp;
        }
        r.peak = pk;
      }
      ...
      if (!exp.getChromatograms().empty())
      {
        ...
        if (options.detailed && chrom_types.contains(ChromatogramSettings::ChromatogramType::SELECTED_REACTION_MONITORING_CHROMATOGRAM))
        {
          os << '\n'
    ...
    if (options.processing)
    {
      ...
      {
        ProcessingStep ps;
        ps.software_name = d.getSoftware().getName();
        ps.software_version = d.getSoftware().getVersion();
        ps.completion_time = d.getCompletionTime().get();
        for (const auto& act : d.getProcessingActions())
        {
          ps.actions.push_back(DataProcessing::NamesOfProcessingAction[act]);
        }
        r.processing.push_back(ps);
      }
    }
