...
namespace OpenMS
{
IsotopeModel::IsotopeModel(): InterpolationModel(), charge_(0), monoisotopic_mz_(0.0)
{
  ...
  defaults_.setValue("isotope:trim_right_cutoff", 0.001,
                     "Cutoff in averagine distribution, trailing isotopes below this relative intensity are not considered.", {"advanced"});
  ...
  defaults_.setValue(
    "isotope:mode:LorentzFWHM", 0.3,
    "Full width of the Lorentzian (Cauchy) function applied to the averagine isotopic pattern to simulate the inaccuracy of the mass spectrometer.",
    {"advanced"});
  defaults_.setValue("isotope:mode:GaussianSD", 0.1,
                     "Standard deviation of Gaussian applied to the averagine isotopic pattern to simulate the inaccuracy of the mass spectrometer.",
                     {"advanced"});
...
IsotopeModel::IsotopeModel(const IsotopeModel& source): InterpolationModel(source)
{
...
void IsotopeModel::setSamples(const EmpiricalFormula& formula)
{
  ...
  */
  ...
  if (param_.getValue("isotope:mode:mode") == "Gaussian")
  {
    ...
    for (double coord = -peak_width; coord <= peak_width; coord += interpolation_step_)
    {
