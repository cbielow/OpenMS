...
RawTandemMSSignalSimulation::RawTandemMSSignalSimulation(const RawTandemMSSignalSimulation& source): DefaultParamHandler(source)
{
...
void RawTandemMSSignalSimulation::initParam_()
{
  ...
  defaults_.setValue("MS_E:add_single_spectra", "false",
                     "If true, the MS2 spectra for each peptide signal are included in the output (might be a lot). They will have a meta value "
                     "'MSE_DebugSpectrum' attached, so they can be filtered out. Native MS_E spectra will have 'MSE_Spectrum' instead.");
  defaults_.setValidStrings("MS_E:add_single_spectra", {"true", "false"});
  defaults_.setValue("tandem_mode", 0,
                     "Algorithm to generate the tandem-MS spectra. 0 - fixed intensities, 1 - SVC prediction (abundant/missing), 2 - SVR prediction "
                     "of peak intensity \n");
  defaults_.setMinInt("tandem_mode", 0);
  defaults_.setMaxInt("tandem_mode", 2);
  defaults_.setValue("svm_model_set_file", "SIMULATION/SvmModelSet.model",
...
void RawTandemMSSignalSimulation::generateMSESpectra_(const SimTypes::FeatureMapSim& features,
                                                      ...
                                                      SimTypes::MSSimExperiment& ms2)
{
  ...
  for (Size i_f = 0; i_f < features.size(); ++i_f)
  {
    ...
    if (tandem_mode && svm_model_charges.count(prec_charge)) { svm_spec_gen_set.simulate(tmp_spec, seq, rnd_gen_->getBiologicalRng(), prec_charge); }
    else { simple_generator.getSpectrum(tmp_spec, seq, 1, prec_charge); }
    ...
    if (! features[i_f].metaValueExists("elution_profile_bounds") || ! features[i_f].metaValueExists("elution_profile_intensities"))
    {
  ...
  {                                            // check which features elute in the current MS scan
    ...
    {
      const DoubleList& elution_bounds = features[i_f].getMetaValue("elution_profile_bounds");
      if ((elution_bounds[1] <= experiment[i].getRT()) && (experiment[i].getRT() <= elution_bounds[3])) { features_fragmented.push_back(i_f); }
    }
    ...
    if (MS2_spectra.size() != 1) { throw Exception::InvalidSize(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, MS2_spectra.size()); }
...
void RawTandemMSSignalSimulation::generateRawTandemSignals(const SimTypes::FeatureMapSim& features,
