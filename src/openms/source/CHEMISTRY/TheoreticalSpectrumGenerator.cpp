...
TheoreticalSpectrumGenerator::TheoreticalSpectrumGenerator(): DefaultParamHandler("TheoreticalSpectrumGenerator")
{
  defaults_.setValue("isotope_model", "none",
                     "Model to use for isotopic peaks ('none' means no isotopic peaks are added, 'coarse' adds isotopic peaks in unit mass distance, "
                     "'fine' uses the hyperfine isotopic generator to add accurate isotopic peaks. Note that adding isotopic peaks is very slow.");
  ...
  defaults_.setValue("add_abundant_immonium_ions", "false",
  ...
  defaults_.setValue("add_z_ions", "false",
                     "Add peaks of z-ions to the spectrum (sometimes observed in CID and for some AAs in ExD due to H abstraction)");
  defaults_.setValidStrings("add_z_ions", {"true", "false"});

  defaults_.setValue("add_zp1_ions", "false",
                     "Add peaks of z+1-radical cations (also [z+H]*^{+} or simply z*) to the spectrum (often observed in ExD)");
  defaults_.setValidStrings("add_zp1_ions", {"true", "false"});

  defaults_.setValue("add_zp2_ions", "false",
                     "Add peaks of z+2-radical cations (also [z+2H]*^{2+} or simply z') to the spectrum (often observed in ExD esp. with higher "
                     "precursor charges >3 and smaller z-ions.)");
...
TheoreticalSpectrumGenerator::TheoreticalSpectrumGenerator(const TheoreticalSpectrumGenerator& rhs): DefaultParamHandler(rhs)
{
}
                                               ...
                                               Int precursor_charge) const
{
  ...
  if (add_metainfo_)
  {
    if (spectrum.getIntegerDataArrays().empty()) { spectrum.getIntegerDataArrays().push_back(std::move(*charges)); }
    if (spectrum.getStringDataArrays().empty()) { spectrum.getStringDataArrays().push_back(std::move(*ion_names)); }
  }
  ...
  if (precursor_charge == 0) { precursor_charge = max_charge + 1; }

  if (precursor_charge < max_charge)
  {
    throw Exception::InvalidParameter(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION,
                                      "'precursor_charge' has to be higher than or equal to 'max_charge'.");
  }
...
MSSpectrum TheoreticalSpectrumGenerator::generateSpectrum(const Precursor::ActivationMethod& fm, const AASequence& seq, int precursor_charge)
{
  ...
  if (fm == Precursor::ActivationMethod::CID || fm == Precursor::ActivationMethod::HCID || fm == Precursor::ActivationMethod::HCD)
  {
    ...
    if (fm == Precursor::ActivationMethod::HCD || fm == Precursor::ActivationMethod::HCID) { theo_gen_settings.setValue("add_a_ions", "true"); }
  }
  ...
  else { throw Exception::InvalidParameter(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, "Fragmentation method is not supported."); }
...
void TheoreticalSpectrumGenerator::addAbundantImmoniumIons_(PeakSpectrum& spectrum,
                                                            const AASequence& peptide,
                                                            DataArrays::StringDataArray& ion_names,
                                                            DataArrays::IntegerDataArray& charges) const
{
                                             ...
                                             Int charge) const
{
  ...
  switch (res_type)
  {
    ...
    case Residue::BIon:
      ...
      break;
    case Residue::CIon:
      if (peptide.size() < 2) throw Exception::InvalidSize(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION, 1);
  ...
  if (add_losses_ && add_metainfo_)
  {
    for (auto& p : peptide)
    {
      for (auto& formula : p.getLossFormulas())
      {
        ...
        if (loss_name.empty()) { loss_name = formula.toString(); }
      }
    }
  }
  ...
  {
    if (peptide.hasNTerminalModification()) { mono_weight += peptide.getNTerminalModification()->getDiffMonoMass(); }
    ...
    if (! add_isotopes_) // add single peak
    {
      ...
      if (i == 1)
      {
        ...
        {
          for (const auto& formula : peptide[0].getLossFormulas())
      ...
      if (add_losses_)
      {
        for (i = Size(! add_first_prefix_ion_); i < peptide.size() - 1; ++i)
        {
          ...
          addLossesFaster_(spectrum, mono_weight + ion_offset, fx_losses, i + 1, ion_names, charges, formula_str_cache,
                           intensity * rel_loss_intensity_, res_type, add_metainfo_, charge);
  ...
  {
    if (peptide.hasCTerminalModification()) { mono_weight += peptide.getCTerminalModification()->getDiffMonoMass(); }
    double initial_mono_weight(mono_weight);
    ...
    if (! add_isotopes_) // add single peak
    {
      ...
      if (add_losses_)
      {
        ...
        for (Size i = peptide.size() - 1; i > 0; --i)
        {
          ...
          addLossesFaster_(spectrum, mono_weight + ion_offset, fx_losses, peptide.size() - i, ion_names, charges, formula_str_cache,
                           intensity * rel_loss_intensity_, res_type, add_metainfo_, charge);
...
void TheoreticalSpectrumGenerator::updateMembers_()
{
  ...
  else if (param_.getValue("isotope_model") == "fine")
    isotope_model_ = 2;
