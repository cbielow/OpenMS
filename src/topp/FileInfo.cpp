...
#include <OpenMS/CONCEPT/LogStream.h>     // getGlobalLogInfo()
#include <OpenMS/FORMAT/FileHandler.h>    // FileHandler::getType()
#include <OpenMS/FORMAT/FileInfo.h>       // the actual inspection logic
...
protected:
  ...
  ExitCodes outputTo_(ostream& os, ostream& os_tsv)
  {
    ...
    opt.detect_labelling = getFlag_("detect_labelling");
    ...
    // labelling detection requires peak data with spectra; a missing result means incompatible input
    if (opt.detect_labelling && !r.labelling.has_value())
    {
